...
import { internSignInSchema } from "@/lib/schema";
...
const InternSignInForm = () => {
  const [loading, setLoading] = useState<boolean>(false);

  const router = useRouter();
  const setIntern = useInternStore((state) => state.setIntern);

  const form = useForm<z.infer<typeof internSignInSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof internSignInSchema>) => {
    setLoading(true);
    try {
      ...
      if (response.status === 200) {
        ...
        router.push("/dashboard/intern/home");
      }
    } catch (error) {
      setLoading(false);
        ...
        <div className="flex justify-center">
          <div className="flex justify-start mt-4 md:w-[400px] text-xs">
            ...
            <div className="ml-3 font-bold">
              <Link href="/intern/signup" className="hover:text-blue-400">
                {" "}
                Sign up
              </Link>
...
export default InternSignInForm;
