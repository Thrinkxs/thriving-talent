...
import { employerSignUpSchema } from "@/lib/schema";
...
export default function EmployerSignUpForm() {
  ...
  const methods = useForm<z.infer<typeof employerSignUpSchema>>({
  ...
  const nextStep = async () => {
    let stepFields: (keyof z.infer<typeof employerSignUpSchema>)[] = [];
  ...
  const onSubmit = async (formValues: z.infer<typeof employerSignUpSchema>) => {
    setLoading(true);
    try {
      ...
      if (response.status === 201) {
        ...
        Cookies.set("role", UserRole.EMPLOYER);
